# Calicut Weather Prediction (Google Colab)

This repository contains a Google Colab-ready workflow for predicting daily weather in Calicut (Kozhikode), Kerala using free weather data from the internet.

## What it does

- Downloads historical daily weather data for Calicut from the Open-Meteo Archive API.
- Downloads the next 16 days of forecast dates from the Open-Meteo Forecast API.
- Builds time-series features such as lag values, rolling averages, day of year, and month.
- Trains machine-learning models for:
  - maximum temperature (`temperature_2m_max`)
  - minimum temperature (`temperature_2m_min`)
  - rainfall / precipitation sum (`precipitation_sum`)
- Prints evaluation metrics and produces forecast plots.

## One-cell Google Colab code

Copy the full cell below into a new Google Colab notebook and run it. It installs dependencies, fetches data from the internet, trains the models, and prints predictions for Calicut.

```python
# Calicut / Kozhikode weather prediction with free internet data (Open-Meteo)
...
This is a learning-friendly weather prediction project, not a replacement for official forecasts or weather warnings. For safety-critical decisions, use official meteorological sources.
